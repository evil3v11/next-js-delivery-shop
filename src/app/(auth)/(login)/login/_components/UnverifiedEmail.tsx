...
interface UnverifiedEmailProps {
  email: string;
  setLoginAction: (email: string) => void;
  setShowUnverifiedEmailAction: (show: boolean) => void;
}
        ...
        <div className="flex flex-col gap-y-3">
          ...
          >
            <span className="flex items-center justify-center gap-2 text-white font-medium py-3.5 px-6 text-sm">
              ...
              <span className="translate-y-px">Подвердить и войти заново</span>
            </span>
          </button>
          <button
            onClick={() => {
            ...
            className={`${formStyles.loginLink} [&&]:h-auto [&&]:my-auto text-primary w-full cursor-pointer`}
          >
            <span className="relative flex items-center justify-center gap-2 font-medium py-3.5 px-6 text-sm hover:*:text-white">
              <Search className="h-5 w-5 text-primary hover:text-white transition-colors duration-300" />
