...
import { getStatusColorClass } from "../_utils/getStatusColorClass";
...
const StatusDropdown = ({
  ...
  const currentStatusData = CUSTOMER_STATUSES.find(status => status.label === currentStatusLabel)
        ...
        <div className="absolute top-full right-0 mt-1 bg-white border border-gray-200 rounded shadow-lg z-200 w-50">
          {CUSTOMER_STATUSES.map((status) => (
            <button
              ...
              className={`flex items-center gap-3 w-full px-4 py-2 text-left hover:bg-[#fcd5ba] duration-300 
              cursor-pointer ${getStatusColorClass(status.label, false)}`}
            >
              <Image
                src={status.icon}
                alt={status.label}
