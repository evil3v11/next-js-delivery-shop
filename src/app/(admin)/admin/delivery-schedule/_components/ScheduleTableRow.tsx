import SelectedIcon from "@/app/(cart)/cart/_components/IconSelect";
    ...
    <div className="grid grid-cols-4 hover:bg-gray-50 transition-colors">
      <div className="p-2 md:p-3 border-r border-gray-200 flex flex-col justify-between">
        ...
        <button
          ...
          className={`${buttonStyles.active} text-[10px] md:text-sm p-1 cursor-pointer rounded`}
        >
          Удалить слот
        </button>
      </div>
          ...
          <div className="flex flex-col items-center gap-1">
            <label className="flex items-center cursor-pointer">
              <input
                ...
                onChange={(e) => onUpdateTimeSlotStatus(date, timeSlot, e.target.checked)}
                className="hidden"
              />
              <span
                className={`w-6 h-6 border-2 rounded flex items-center justify-center duration-300
            ...
            <span
              className={`text-xs md:text-sm ${schedule[date]?.[timeSlot] !== false ? "text-primary" : "text-[#d80000]"}`}
            >
