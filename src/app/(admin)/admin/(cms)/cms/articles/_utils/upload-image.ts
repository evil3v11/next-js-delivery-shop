...
import { TempImageUploadResult } from "../_types";
...
export const validateImageFile = (file: File): string | null => {
  ...
  if (file.size > maxSize) return `Файл слишком большой. Максимальный размер: 5MB.`;

  return null;
};

export const uploadToServer = async (file: File): Promise<TempImageUploadResult> => {
  const formData = new FormData();
  formData.append("image", file);

  const response = await fetch("/admin/cms/api/articles/temp-image", {
      method: "POST",
      body: formData,
    },
  );
...
const getInsertPosition = (editor: Editor): number => {
  ...
  if (from !== to) return Math.max(from, to);
