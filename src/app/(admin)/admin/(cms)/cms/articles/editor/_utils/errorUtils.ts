import { ErrorWithStatusCode } from "../../_types";
...
export const getErrorMessage = (statusCode?: number): string => {
  if (!statusCode) return "Ошибка подключения к YandexGPT";
  ...
  switch (statusCode) {
    ...
    default:
      if (statusCode >= 400 && statusCode < 500) return `Ошибка клиента YandexGPT (${statusCode})`;
      if (statusCode >= 500 && statusCode < 600) return `Ошибка сервера YandexGPT (${statusCode})`;
