...
const ArticleSubmitSection = ({ onCancel }: { onCancel: () => void }) => {
  const { formData, updateFormField, isSubmitting, isUploading } =
    useArticleStore();
  ...
  useEffect(() => {
    ...
    if (formData.isFeatured) setIsFeatured(formData.isFeatured);
  }, [formData.status, formData.isFeatured]);

  const handleStatusChange = (status: ArticleStatus): void => {
    setArticleStatus(status);
    updateFormField("status", status);
  };

  const handleFeaturedChange = (isFeatured: boolean): void => {
    setIsFeatured(isFeatured);
    updateFormField("isFeatured", isFeatured);
  };

  const handleCancelWithConfirm = (): void => {
    const hasData = !!(
      formData.name.trim() ||
      formData.slug.trim() ||
    ...
    if (hasData) {
      const confirmCancel = confirm(
        "Вы уверены, что хотите отменить создание статьи? Все введенные данные будут утеряны",
      );
  ...
  const canShowPreview = !!formData.content?.trim();

  return (
    <>
      {showPreview && (
        <ArticlePreviewModal onClose={() => setShowPreview(false)} />
      )}
      <div className="mb-6 bg-linear-to-r from-purple-50 to-indigo-50 p-4 rounded-xl border border-purple-200">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <button
            ...
            disabled={!canShowPreview || isUploading || isSubmitting}
