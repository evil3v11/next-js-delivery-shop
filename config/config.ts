export const CONFIG = {
  ITEMS_PER_PAGE: 3,
  ITEMS_PER_PAGE_MAIN_PRODUCTS: 4,
  ITEMS_PER_PAGE_MAIN_ARTICLES: 3,
  ITEMS_PER_PAGE_CATEGORY: 6,
  ITEMS_PER_ORDERS_PAGE: 4,
  ARTICLES_PER_BLOG_PAGE: 10,
  RELATED_ARTICLES_PER_ARTICLE: 3,
  COMMENTS_PER_ARTICLE_PAGE: 3,
  CARD_DISCOUNT_PERCENT: 6,
  PRODUCT_BONUSES_PERCENT: 5,
  MAX_BONUSES_PERCENTAGE: 10,
  FALLBACK_PRICE_RANGE: { min: 0, max: 3000 },
  SITE_URL: "https:/delivery-shop.ru",
  TEMP_EMAIL_DOMAIN: "@delivery-shop.ru",
  MAX_ATTEMPTS: 3,
  TIMEOUT_PERIOD: 180,
  DEFAULT_PAGE_SIZE: 5,
  PAGE_SIZE_OPTIONS: [1, 5, 10, 20, 50, 100],
  MAX_STAR_AMOUNT: 5,
  MESSAGE_TIMEOUT_DELAY: 3000,
};
